...
export default function Dashboard() {
  ...
  async function searchPexels(query: string) {
    ...
    } catch { console.error('Pexels error') }
  ...
  async function handleMagicCommand() {
    ...
    try {
      const res = await fetch('/api/agent', {
        ...
        body: JSON.stringify({ messages: [{ role: 'user', text: magicCommand }], script, idea, style })
      })
      const data = await res.json()
      if (data.reply) setMagicResult(data.reply)
    } catch { setMagicResult('⚠️ Try again!') }
                  ...
                  <div className="grid grid-cols-5 gap-2">
                    {styles.map(s => (
                      <button key={s.id} onClick={() => setStyle(s.id)}
                        className={`flex flex-col items-center gap-1 p-2 sm:p-3 rounded-xl border text-center transition-all ${style === s.id ? 'bg-[#00c8ff]/10 border-[#00c8ff]' : 'border-white/10 text-gray-400 hover:border-white/30'}`}>
              ...
              <div className="max-w-3xl mx-auto">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-gray-400 text-xs mt-0.5">"{idea.slice(0, 40)}{idea.length > 40 ? '...' : ''}"</p>
                  </div>
                ...
                {/* ═══ VIDEO OUTPUT PANEL (TOP) ═══ */}
                <div className="bg-white/5 border border-white/10 rounded-2xl p-4 mb-4">

                  {/* Screen size selector inside panel */}
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-xs text-gray-400 font-semibold uppercase tracking-wider">📐 Screen Size</span>
                    <div className="flex gap-1.5">
                      {aspectRatios.map(r => (
                        <button key={r.ratio} onClick={() => setAspectRatio(r.ratio)}
                          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold border transition-all ${aspectRatio === r.ratio ? 'bg-[#00c8ff]/20 border-[#00c8ff] text-[#00c8ff]' : 'border-white/10 text-gray-400 hover:border-white/30'}`}>
                          <span>{r.icon}</span>
                          <span>{r.ratio}</span>
                        </button>
                      ))}
                  ...
                  <div className="flex justify-center mb-4">
                    <div className="relative" style={currentAspect.style as any}>
                      <div className={`w-full h-full rounded-2xl overflow-hidden border-2 border-gray-700 shadow-2xl relative ${aspectRatio === '9:16' ? 'rounded-[1.5rem]' : ''}`}
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center z-10">
                          ...
                          <div className="text-xs font-bold text-white text-center px-3 leading-tight">{idea.slice(0, 40)}</div>
                        </div>
                        ...
                        <div className="absolute bottom-6 left-2 right-2 text-center z-10">
                          <span className={`font-black leading-tight ${captionSize === 'large' ? 'text-sm' : captionSize === 'medium' ? 'text-xs' : 'text-[10px]'}`}
                            style={{ color: captionColor, fontFamily: brandFont, textShadow: '0 2px 8px rgba(0,0,0,0.8)' }}>
                            The REAL reason...
                          </span>
                        </div>
                        ...
                        <div className="absolute top-2 right-2 z-20 flex flex-col gap-1">
                          <div className="bg-black/60 backdrop-blur-sm rounded-lg px-2 py-1 text-center">
                            <div className="text-xs font-black text-[#00c8ff]">88</div>
                            <div className="text-[8px] text-gray-400">Score</div>
                          </div>
                          <div className="bg-black/60 backdrop-blur-sm rounded-lg px-2 py-1 text-center">
                            <div className="text-xs font-black text-green-400">58s</div>
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div className="flex-1 h-1.5 bg-white/10 rounded-full relative">
                      ...
                      <div className="absolute top-1/2 left-1/3 -translate-y-1/2 w-3 h-3 bg-white rounded-full -ml-1.5 shadow-lg" />
                    </div>
                    <span className="text-xs text-gray-400 shrink-0">0:19 / 0:58</span>
                    {/* Volume */}
                    <div className="flex items-center gap-1.5 shrink-0">
                      <span className="text-xs">🔊</span>
                      <input type="range" min="0" max="100" value={volume} onChange={e => setVolume(+e.target.value)}
                        className="w-16 accent-[#00c8ff]" />
                    </div>
                  </div>

                  {/* ✂️ TRIM — embedded in video panel */}
                  <div className="border-t border-white/10 pt-4">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <span className="text-xs text-gray-400">
                        ...
                        <span className="text-[#00c8ff] ml-2 font-semibold">({Math.round((trimEnd - trimStart) * 0.58)}s clip)</span>
                      </span>
                    </div>
                    {/* Timeline */}
                    <div className="relative h-10 bg-white/5 rounded-xl overflow-hidden mb-2">
                      <div className="absolute inset-y-0 bg-[#00c8ff]/10 border-x-2 border-[#00c8ff] rounded-lg"
                    ...
                    <div className="relative h-2 mb-3">
                      <input type="range" min="0" max="100" value={trimStart}
                        ...
                        className="absolute inset-0 w-full opacity-0 cursor-pointer z-10 h-2" />
                      <input type="range" min="0" max="100" value={trimEnd}
                        onChange={e => setTrimEnd(Math.max(+e.target.value, trimStart + 5))}
                        className="absolute inset-0 w-full opacity-0 cursor-pointer z-20 h-2" />
                      <div className="absolute inset-0 bg-white/5 rounded-full">
                        <div className="absolute inset-y-0 bg-gradient-to-r from-[#00c8ff] to-[#7b2fff] rounded-full"
                          style={{ left: `${trimStart}%`, right: `${100 - trimEnd}%` }} />
                        <div className="absolute top-1/2 -translate-y-1/2 w-4 h-4 bg-[#00c8ff] rounded-full shadow-lg border-2 border-white"
                          style={{ left: `calc(${trimStart}% - 8px)` }} />
                        <div className="absolute top-1/2 -translate-y-1/2 w-4 h-4 bg-[#7b2fff] rounded-full shadow-lg border-2 border-white"
                          style={{ left: `calc(${trimEnd}% - 8px)` }} />
                      </div>
                    </div>
                    <div className="flex justify-between text-[10px] text-gray-500">
                      <span>0:00</span><span>0:15</span><span>0:30</span><span>0:45</span><span>0:58</span>
                    </div>
                  </div>
                </div>

                {/* ═══ EDITOR TABS (BELOW VIDEO) ═══ */}
                  ...
                  <div className="flex overflow-x-auto scrollbar-hide border-b border-white/10 bg-white/[0.02]">
                    {editorTabs.map(tab => (
                      <button key={tab.id} onClick={() => setActiveEditorTab(tab.id as any)}
                        className={`px-4 py-3 text-xs font-semibold whitespace-nowrap transition-all border-b-2 shrink-0 ${
                          activeEditorTab === tab.id
                            ? 'border-[#00c8ff] text-white bg-[#00c8ff]/5'
                            : 'border-transparent text-gray-400 hover:text-white hover:bg-white/5'
                        } ${tab.id === 'export' ? 'ml-auto' : ''}`}>
                        {tab.label}
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="p-4">
                    ...
                    {activeEditorTab === 'video' && (
                      <div className="space-y-3">
                        <div>
                          <label className="text-xs text-gray-400 font-semibold block mb-2">✍️ Script Editor</label>
                        ...
                        <div className="flex items-center justify-between">
                          <label className="text-xs text-gray-400 font-semibold">⚡ Speed</label>
                          <div className="flex gap-1.5">
                            {[0.5, 0.75, 1, 1.25, 1.5, 2].map(s => (
                              <button key={s} onClick={() => {}}
                                className="px-2.5 py-1 rounded-full text-xs font-bold border border-white/10 text-gray-400 hover:border-[#00c8ff]/50 transition-all">
                                {s}x
                              </button>
                            ))}
                          </div>
                    ...
                    {activeEditorTab === 'captions' && (
                      <div className="space-y-4">
                        ...
                        <div>
                          <label className="text-xs text-gray-400 font-semibold block mb-2">Color</label>
                    ...
                    {activeEditorTab === 'voice' && (
                      <div className="space-y-4">
                        ...
                        <div>
                          <div className="flex justify-between text-xs mb-1"><span className="text-gray-400">Pitch</span><span>{voicePitch}%</span></div>
                          <input type="range" min="0" max="100" value={voicePitch} onChange={e => setVoicePitch(+e.target.value)} className="w-full accent-[#00c8ff]" />
                        </div>
                        <div>
                          <div className="flex justify-between text-xs mb-1"><span className="text-gray-400">Speed</span><span>{voiceSpeed}%</span></div>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="p-3 bg-[#00c8ff]/5 border border-[#00c8ff]/20 rounded-xl">
                          <p className="text-xs text-[#00c8ff] font-semibold">
                            ✓ {languages.find(l => l.code === selectedLanguage)?.name} · {accents.find(a => a.id === selectedAccent)?.label} accent
                          </p>
                        </div>
                      </div>
                    )}
                      ...
                      <div className="space-y-3">
                        <div className="flex gap-2">
                          ...
                          <button onClick={() => searchPexels(pexelsQuery)} disabled={pexelsLoading}
                            className="bg-gradient-to-r from-[#00c8ff] to-[#7b2fff] text-black font-bold px-4 rounded-xl text-xs disabled:opacity-50">
                            {pexelsLoading ? '...' : '🔍'}
                          </button>
                        </div>
                        {pexelsResults.length === 0 && !pexelsLoading && (
                          <div className="text-center py-6">
                            ...
                            <p className="text-gray-400 text-xs mb-3">Search Pexels stock library</p>
                            <div className="flex flex-wrap gap-2 justify-center">
                              {['space', 'nature', 'city', 'ocean', 'abstract'].map(q => (
                                <button key={q} onClick={() => { setPexelsQuery(q); searchPexels(q) }}
                                  className="text-xs border border-white/10 rounded-full px-3 py-1 hover:border-[#00c8ff]/50 text-gray-400 transition-all">
                                  {q}
                                </button>
                              ))}
                            </div>
                        ...
                        {pexelsLoading && <div className="text-center py-6 text-gray-400 text-sm">Searching...</div>}
                        {pexelsResults.length > 0 && (
                          <>
                            <div className="grid grid-cols-4 gap-2">
                              {pexelsResults.map(photo => (
                                <button key={photo.id} onClick={() => setSelectedMedia(photo.src.medium)}
                                  className={`relative aspect-square rounded-xl overflow-hidden border-2 transition-all ${selectedMedia === photo.src.medium ? 'border-[#00c8ff]' : 'border-transparent hover:border-white/30'}`}>
                                  <img src={photo.src.small} alt={photo.alt} className="w-full h-full object-cover" />
                                  {selectedMedia === photo.src.medium && (
                                    <div className="absolute inset-0 bg-[#00c8ff]/30 flex items-center justify-center text-lg">✓</div>
                                  )}
                                </button>
                              ))}
                            </div>
                            {selectedMedia && (
                              <div className="p-2 bg-green-500/10 border border-green-500/30 rounded-xl flex items-center gap-2">
                                <span className="text-green-400 text-xs font-semibold">✓ Background selected</span>
                                <button onClick={() => setSelectedMedia(null)} className="ml-auto text-xs text-gray-400">Remove</button>
                              </div>
                            )}
                          </>
                        )}
                        <p className="text-center text-gray-600 text-xs">Powered by Pexels · Free to use</p>
                      </div>
                    )}
                    ...
                    {activeEditorTab === 'brand' && (
                      <div className="space-y-4">
                        <div>
                          <label className="text-xs text-gray-400 font-semibold block mb-2">Brand Color</label>
                          <div className="flex items-center gap-3 mb-2">
                            <input type="color" value={brandColor} onChange={e => setBrandColor(e.target.value)}
                              className="w-10 h-10 rounded-xl border border-white/10 cursor-pointer bg-transparent" />
                            <span className="font-bold text-sm">{brandColor}</span>
                          </div>
                          <div className="flex gap-2">
                        ...
                        <div>
                          <label className="text-xs text-gray-400 font-semibold block mb-2">Brand Font</label>
                          <div className="grid grid-cols-3 gap-2">
                            {['Inter', 'Roboto', 'Montserrat', 'Playfair Display', 'Oswald', 'Poppins'].map(font => (
                              <button key={font} onClick={() => setBrandFont(font)}
                                className={`p-2 rounded-xl border text-xs font-semibold transition-all ${brandFont === font ? 'border-[#00c8ff] bg-[#00c8ff]/10 text-[#00c8ff]' : 'border-white/10 text-gray-300'}`}
                        ...
                        <div>
                          <label className="text-xs text-gray-400 font-semibold block mb-2">Logo URL</label>
                        ...
                        {/* Preview */}
                        <div className="p-4 rounded-xl border border-white/10 aspect-video flex items-center justify-center"
                          style={{ background: `linear-gradient(135deg, ${brandColor}30, #050d1a)` }}>
                          <div className="text-center">
                            <div className="font-black text-xl" style={{ color: brandColor, fontFamily: brandFont }}>Your Channel</div>
                            <div className="text-xs text-gray-400 mt-1" style={{ fontFamily: brandFont }}>Brand Preview</div>
                          </div>
                        </div>
                        <button className="w-full bg-gradient-to-r from-[#00c8ff] to-[#7b2fff] text-black font-black py-3 rounded-xl text-sm">
                          ✓ Apply to All Shorts
                        </button>
                      </div>
                    )}
                    ...
                    {activeEditorTab === 'thumbnail' && (
                      <div className="space-y-3">
                        <div className={`w-full aspect-video rounded-xl flex items-center justify-center relative overflow-hidden ${!thumbImageUrl ? `bg-gradient-to-br ${thumbnailTemplates.find(t => t.id === selectedThumb)?.bg}` : ''}`}>
                          {thumbImageUrl ? (
                            ...
                            <>
                              ...
                              <div className="relative z-10 text-center px-4">
                                <div className="text-white font-black text-lg">{thumbTitle}</div>
                              </div>
                            </>
                          )}
                        </div>
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          {thumbnailTemplates.map(t => (
                            <button key={t.id} onClick={() => { setSelectedThumb(t.id); setThumbImageUrl(null) }}
                              className={`aspect-video rounded-xl bg-gradient-to-br ${t.bg} border-2 transition-all ${selectedThumb === t.id && !thumbImageUrl ? 'border-white' : 'border-transparent'}`} />
                          ))}
                        </div>
                        <div className="flex gap-2">
                          <button onClick={() => fileInputRef.current?.click()}
                            className="flex-1 border border-white/20 py-2.5 rounded-xl text-xs font-semibold hover:border-[#00c8ff]/50 transition-all">
                            📁 Upload
                          </button>
                          <button className="flex-1 bg-gradient-to-r from-[#00c8ff] to-[#7b2fff] text-black font-bold py-2.5 rounded-xl text-xs">
                            ⬇️ Download
                          </button>
                        </div>
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            {[
                              ...
                              { label: 'Thumbnail', badge: 'FREE', color: '#00c8ff', icon: '🖼️' },
                            ].map(opt => (
                              <button key={opt.label}
                                className="flex items-center gap-2 p-3 bg-white/5 border border-white/10 hover:border-white/30 rounded-xl transition-all">
                                <span>{opt.icon}</span>
                                <span className="text-xs font-semibold flex-1 text-left">{opt.label}</span>
                        ...
                        <div className="border-t border-white/10 pt-4">
                          <div className="flex items-center justify-between mb-4">
                            <div>
                              ...
                              <p className="text-xs text-gray-400">Configure before publishing</p>
                            </div>
                          ...
                          {uploadToYT && (
                            <div className="space-y-3">
                              <div>
                                <label className="text-xs text-gray-400 font-semibold block mb-1">📝 Title <span className="text-red-400">*</span></label>
                                ...
                                <div className="text-right text-xs text-gray-600 mt-0.5">{ytTitle.length}/100</div>
                              </div>
                              <div>
                                  ...
                                  <div className="space-y-1.5">
                                    {(['public', 'unlisted', 'private'] as const).map(v => (
                                      <button key={v} onClick={() => setYtVisibility(v)}
                                        className={`w-full flex items-center gap-2 px-3 py-2 rounded-xl border text-xs font-semibold transition-all capitalize ${ytVisibility === v ? 'border-[#00c8ff] bg-[#00c8ff]/10 text-[#00c8ff]' : 'border-white/10 text-gray-400'}`}>
                                  ...
                                  <div className="space-y-1.5">
                                    {['Education', 'Entertainment', 'Science', 'People', 'News'].map(cat => (
                                      <button key={cat} onClick={() => setYtCategory(cat)}
                                        className={`w-full px-3 py-2 rounded-xl border text-xs font-semibold transition-all ${ytCategory === cat ? 'border-[#7b2fff] bg-[#7b2fff]/10 text-[#7b2fff]' : 'border-white/10 text-gray-400'}`}>
                                        {cat}
                                      </button>
                                    ))}
                                  </div>
                              ...
                              <div className="bg-white/[0.03] border border-white/10 rounded-xl p-3 text-xs space-y-1">
                                <div className="font-semibold text-gray-300 mb-2">📋 Summary</div>
                                <div className="flex gap-2"><span className="text-gray-500 w-20">Title:</span><span className="text-white truncate">{ytTitle || '—'}</span></div>
                                <div className="flex gap-2"><span className="text-gray-500 w-20">Visibility:</span><span className="text-[#00c8ff] capitalize">{ytVisibility}</span></div>
                                <div className="flex gap-2"><span className="text-gray-500 w-20">Category:</span><span className="text-white">{ytCategory}</span></div>
                                <div className="flex gap-2"><span className="text-gray-500 w-20">Hashtags:</span><span className="text-[#7b2fff] truncate">{ytHashtags}</span></div>
                              </div>
                              <button disabled={!ytTitle.trim()}
                                className="w-full bg-red-500 hover:bg-red-600 disabled:opacity-40 text-white font-black py-4 rounded-xl text-sm flex items-center justify-center gap-2 transition-all">
                          ...
                          {!uploadToYT && (
                            <div className="text-center py-4 text-gray-400 text-sm">
                              Toggle on to set up YouTube upload
                            </div>
                          )}
                        </div>
                      </div>
            ...
            <div className="space-y-3">
              {recentShorts.map((s, i) => (
                <div key={i} className="bg-white/5 border border-white/10 rounded-2xl p-4 flex items-center gap-3 hover:border-white/20 transition-all">
                  ...
                  <span className={`text-xs font-semibold px-2 py-1 rounded-full ${s.status === 'Published' ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
                    {s.status}
                  </span>
                </div>
              ))}
        ...
        <div className="fixed bottom-24 left-4 sm:left-6 z-40">
          {magicOpen && (
            <div className="mb-3 bg-[#080f1e] border border-[#7b2fff]/50 rounded-2xl p-4 shadow-2xl w-80"
              ...
              <p className="text-xs text-gray-500 mb-3">Type any instruction — AI applies it instantly</p>
              <div className="flex gap-2 mb-3">
                <input value={magicCommand} onChange={e => setMagicCommand(e.target.value)}
                  onKeyDown={e => e.key === 'Enter' && handleMagicCommand()}
                  placeholder='"make hook dramatic"'
                  autoFocus
              ...
              <div className="flex flex-wrap gap-1.5 mb-3">
                {['Make hook dramatic', 'Shorter script', 'Add strong CTA', 'Cinematic style', 'Viral score', 'Better title'].map(cmd => (
              ...
              {magicLoading && (
                <div className="flex items-center gap-2 text-xs text-[#7b2fff] py-1">
              ...
              {magicResult && !magicLoading && (
                <div className="bg-[#7b2fff]/10 border border-[#7b2fff]/30 rounded-xl p-3 max-h-48 overflow-y-auto">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs text-[#7b2fff] font-bold">✨ Result</span>
