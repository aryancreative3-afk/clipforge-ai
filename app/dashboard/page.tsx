    ...
    try {
      ...
      if (data.reply) {
        if (magicCommand.toLowerCase().includes('rewrite') ||
            ...
            magicCommand.toLowerCase().includes('script')) {
          setScript(prev => prev + '\n\n✨ AI Suggestion:\n' + data.reply)
        }
        alert('✨ Magic Box:\n\n' + data.reply)
      }
    } catch {
      alert('Magic Box unavailable. Try again!')
    }
    ...
    <div className="min-h-screen bg-[#050d1a] text-white flex flex-col">
                          ...
                          <div className="mb-4">
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div className="flex-1">
                                ...
                                <div className="text-xs text-gray-400">Used in captions, thumbnails & overlays</div>
                              </div>
                            </div>
                          ...
                          {uploadToYT && (
                            <div className="space-y-3">
                              <input value={ytTitle} onChange={e => setYtTitle(e.target.value)} placeholder="Video title"
                                className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-white text-xs focus:outline-none focus:border-[#00c8ff]/50 transition-colors" />
                              <textarea value={ytDesc} onChange={e => setYtDesc(e.target.value)} rows={2} placeholder="Description"
      ...
      {done && (
        <div className="fixed bottom-24 left-4 sm:left-6 z-40">
          {magicOpen && (
            <div className="mb-3 bg-[#080f1e] border border-[#7b2fff]/50 rounded-2xl p-4 shadow-2xl w-72"
              style={{ boxShadow: '0 0 40px rgba(123,47,255,0.2)' }}>
              <div className="flex items-center gap-2 mb-3">
                <span className="text-lg">🪄</span>
                <span className="font-black text-sm">Magic Box</span>
                <span className="text-xs text-gray-400 ml-auto">Press Enter</span>
              </div>
              <input value={magicCommand} onChange={e => setMagicCommand(e.target.value)}
                onKeyDown={e => e.key === 'Enter' && handleMagicCommand()}
                placeholder='Try "make hook dramatic"'
                autoFocus
                className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-white text-xs focus:outline-none focus:border-[#7b2fff]/50 transition-colors mb-3" />
              <div className="flex flex-wrap gap-1.5">
                {['Make hook dramatic', 'Shorter script', 'Add CTA', 'Cinematic style'].map(cmd => (
                  <button key={cmd} onClick={() => setMagicCommand(cmd)}
                    className="text-xs bg-white/5 border border-white/10 rounded-full px-2.5 py-1 hover:border-[#7b2fff]/50 transition-all text-gray-300">
                    {cmd}
                  </button>
                ))}
              </div>
              {magicLoading && (
                <div className="mt-3 flex items-center gap-2 text-xs text-[#7b2fff]">
          ...
          <button onClick={() => setMagicOpen(!magicOpen)}
            className="w-12 h-12 rounded-full bg-gradient-to-br from-[#7b2fff] to-[#00c8ff] flex items-center justify-center shadow-2xl hover:scale-110 transition-all text-xl"
